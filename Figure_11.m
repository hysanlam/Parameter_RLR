...
%
% Generates Figure 11 from the manuscript.
...
x=([rand(1,100),1.5+rand(1,700),3+rand(1,100)]);
y=rand(1,900);
n0_2=n0*n0;
...
rank_fun=18;
f=chebfun2( @(d,theta) (1/n0_2)*sigma.^2.*exp(-d.^2./(2.*theta.^2)),[0,5,0.2,0.4],rank_fun);
[C,D,R] = cdr(f);
U=C;
V=D*R'
...
for d=1:length(theta_dis)
    tic
        ...
        cov=cov_operator(theta_dis(d),n0,xy);
        fun_val_svd=[fun_val_svd,Error_SVD(cov,rank)];
        [d,toc]
end
for r=1:length(rank)
...
for count=1:20
    ...
    for r=rank
        ...
        for d=1:length(theta_dis)
            cov=cov_operator(theta_dis(d),n0,xy);
            fun_val_GN=[fun_val_GN, (norm(cov-GN(ttv(A_Omega,U(theta_dis(d))',3).data,ttv(A_Psi,U(theta_dis(d))',3).data,Psi),"fro"))^2];
            fun_val_rsvd=[fun_val_rsvd,(norm(cov-RSVD_linear(Q,Y_Omega_1,Z_Omega_1,U(theta_dis(d))),"fro"))^2];     
...
function C=cov_operator(theta,n0,xy)
    C=zeros(n0^2,n0^2);
    for i=1:n0^2
         C(:,i)=(1./(n0*n0))*exp(-xy(:,i).^2./(2.*theta.^2));
    end
end
